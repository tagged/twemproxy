...
mbuf_free(struct mbuf *mbuf)
{
    ...
    log_debug(LOG_VVERB, "put mbuf %p len %ld", mbuf, mbuf->last - mbuf->pos);
...
mbuf_insert(struct mhdr *mhdr, struct mbuf *mbuf)
{
    ...
    log_debug(LOG_VVERB, "insert mbuf %p len %ld", mbuf, mbuf->last - mbuf->pos);
}
...
{
    log_debug(LOG_VVERB, "remove mbuf %p len %ld", mbuf, mbuf->last - mbuf->pos);
